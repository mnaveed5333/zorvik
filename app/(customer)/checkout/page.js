...
export default function CheckoutPage() {
  ...
  // Quantity chosen on the product page arrives via the URL — fall back to 1 if missing/invalid
  ...
  // Generated once per checkout session — this is the single order ID
  // used on the receipt, sent to the backend, and shown on the vendor's order card.
  const [orderId] = useState(() => generateOrderId());

  // Customer can type either 10 digits (3001234567) or 11 with a leading zero
  // (03001234567) — the form shows exactly what they typed, but the leading
  // zero is dropped when we build the actual number used everywhere else.
    ...
    const fetchProduct = async () => {
      try {
        ...
        if (data.success) {
          ...
          // Clamp the URL-provided quantity to available stock, once we know it
  ...
  // Allows 10 digits when the number doesn't start with 0 (3001234567),
  // or up to 11 digits when it does (03001234567) — cap adjusts as the
  // user types based on whatever the first digit turns out to be.
  ...
  const handleOrderOnWhatsapp = async () => {
    ...
    try {
      ...
      const { data } = await axios.post("/api/orders", {
        orderId, // send the generated ID so backend stores/returns the same one
        customerName: name,
  ...
  return (
    <div className="bg-white min-h-screen">
      <div className="max-w-md mx-auto px-4 py-8">
        ...
        {/* Hidden receipt template, captured by html2canvas-pro.
            Kept pixel-identical to the vendor's print receipt in OrderCard.jsx (same 380px width,
            same font sizes, same section order) — except no PAID stamp, since that only shows
